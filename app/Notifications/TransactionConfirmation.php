...
class TransactionConfirmation extends Notification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->line(sprintf('Howdy %s, your %s transaction of %s %s has been completed',
                explode(' ', $notifiable->name)[0], $this->transaction->type, currency($this->transaction->amount, true, 8), $this->transaction->item))
