...
if (!function_exists('trader_club_mail')) {
    function trader_club_mail()
    {
        config(['mail.from.address' => "support@trader-club.net"]);
        config(['services.mailgun.domain' => "trader-club.net"]);
        $app = app();
...
if (!function_exists('pauls_club_mail')) {
    function pauls_club_mail()
    {
        config(['mail.from.address' => "support@pauls-club.net"]);
        config(['services.mailgun.domain' => "pauls-club.net"]);
        $app = app();
        foreach (['mailer', 'swift.mailer', 'swift.transport'] as $service) {
            unset($app[$service]);
        }
...
if (!function_exists('file_in_dir')) {
    function files_in_dir($path, array $extensions)
    {
        $files = [];
        $dir = new DirectoryIterator($path);
        foreach ($dir as $fileinfo) {
            if ($fileinfo->isDir() && !$fileinfo->isDot()) {
                $files = array_merge($files, files_in_dir($fileinfo->getRealPath(), $extensions));
        ...
        $html = '<div class="form-group">
                ...
                <div class="input-group date" id="datetimepicker' . $id . '"  data-date-format="YYYY-MM-DD HH:mm:ss" data-target-input="nearest">
                    ...
                    <div class="input-group-append" data-target="#datetimepicker' . $id . '" data-toggle="datetimepicker">
                                                  <button class="btn btn-primary" type="button"><i class="fe fe-calendar"></i></button>
